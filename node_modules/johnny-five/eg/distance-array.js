...
board.on("ready", function() {
  ...
  var directions = [
    { position: "right", model: "0A21", range: [ 6, 7 ] },
    { position: "center", model: "0A02", range: [ 24, 36 ] },
    { position: "left", model: "0A21", range: [ 6, 9 ] },
  ];
  ...
  var machine = StateMachine.create({
    ...
    events: [
      { name: "move", from: ["moving", "avoiding", "stopped"], to: "moving" },
      { name: "avoid", from: "moving", to: "avoiding"  },
      { name: "stop", from: ["moving", "avoiding"], to: "stopped" },
    ],
        ...
        if (sensor[position].isSampled) {
          if (distance !== 0 && distance <= unsafe) {
            if (machine.can("avoid")) {
              ...
              machine.avoid({ direction: alternative });
